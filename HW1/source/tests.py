...
for i in range(9, 20):
    ...
    I = []
    for y, row in enumerate(problem["map"]):
        for x, col in enumerate(row):
            if col == "I":
                I.append((y, x))
    flag = False
    for name, data in problem["passengers"].items():
        if (data["destination"] in I) or (data["location"] in I):
            flag = True
            break
    if flag:
        print(i + 1, "Unsolvable")
        continue
    ...
    if isinstance(s, search.Node):
        solve = s
        solution = list(map(lambda n: n.action, solve.path()))[1:]
        print(i + 1, len(solution), t2 - t1, solution)
